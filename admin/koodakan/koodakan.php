		...
		case UPLOAD_ERR_OK:
			...
			if ($valid && $typimg) {
				...
				move_uploaded_file($tmpName,$targetPath);
					$result =mysqli_query($_SESSION['connect'],"INSERT INTO koodakan (img_name,img_desc) VALUES ('".$imagename."','".$_POST['desc']."')");
...
if(isset($_POST['inx']) )
{
	...
	{
			 $maindir = "../slide/koodakan/" ;
				$mydir = opendir($maindir) ;
				$fake = array();
				$filePaths = array();
				$exclude = array( ".", "..", "index.php",".htaccess","koodakan.php") ;
				while($fn = readdir($mydir))
				{
								if (!in_array($fn, $exclude)) 
								{
				...
				closedir($mydir);
			 $qry =mysqli_query($_SESSION['connect'],"SELECT * FROM `koodakan` WHERE id='".$_POST['shakhes']."'");
				$erw = mysqli_fetch_array($qry);
				thumb_create($erw['img_name'],'1234','800');
			 header( 'Location: ../koodakan.php' ) ;
				exit;
...
function thumb_create($file, $width , $height ) {
        try
        {
                ...
                $im->writeImage( '../slide/koodakan/THUMB_'.$file );
