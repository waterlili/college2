		...
		case UPLOAD_ERR_OK:
			...
			if ($valid && $typimg) {
				...
				move_uploaded_file($tmpName,$targetPath);
					$result =mysqli_query($_SESSION['connect'],"INSERT INTO myworks (img_name,img_desc) VALUES ('".$imagename."','".$_POST['desc']."')");
...
if(isset($_POST['inx']) )
{
	if(isset($_POST['shakhes']))
	{
			 $maindir = "../slide/myworks/" ;
				...
				$fake = array();
				$filePaths = array();
				$exclude = array( ".", "..", "index.php",".htaccess","my-task.php","removetask.php") ;
				while($fn = readdir($mydir))
				{
								if (!in_array($fn, $exclude)) 
								{
				...
				closedir($mydir);
			 $qry =mysqli_query($_SESSION['connect'],"SELECT * FROM `myworks` WHERE id='".$_POST['shakhes']."'");
				$erw = mysqli_fetch_array($qry);
				thumb_create($erw['img_name'],'1234','800');
			 header( 'Location: ../my-tasks.php' ) ;
				exit;
