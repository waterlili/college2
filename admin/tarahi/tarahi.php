		...
		case UPLOAD_ERR_OK:
			...
			if ($valid && $typimg) {
				...
				move_uploaded_file($tmpName,$targetPath);
					$result =mysqli_query($_SESSION['connect'],"INSERT INTO tarahi (img_name,img_desc) VALUES ('".$imagename."','".$_POST['desc']."')");
...
if(isset($_POST['inx']) )
{
	...
	{
			 $maindir = "../slide/tarahi/" ;
				$mydir = opendir($maindir) ;
				$fake = array();
				$filePaths = array();
				$exclude = array( ".", "..", "index.php",".htaccess","tarahi.php") ;
				while($fn = readdir($mydir))
				{
								if (!in_array($fn, $exclude)) 
								{
				...
				closedir($mydir);
			 $qry =mysqli_query($_SESSION['connect'],"SELECT * FROM `tarahi` WHERE id='".$_POST['shakhes']."'");
				$erw = mysqli_fetch_array($qry);
				thumb_create($erw['img_name'],'1234','800');
			 header( 'Location: ../tarahi.php' ) ;
				exit;
...
function thumb_create($file, $width , $height ) {
        try
        {
                ...
                $im->writeImage( '../slide/tarahi/THUMB_'.$file );
